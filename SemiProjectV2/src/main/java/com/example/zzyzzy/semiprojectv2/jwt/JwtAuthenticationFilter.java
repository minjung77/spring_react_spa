package com.example.zzyzzy.semiprojectv2.jwt;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    //클라이언트의 http 요청이 들어오면 필터 체인에 등록됨
    //JwtAuthenticationFilter 의 doFilterInternal 메서드가 호출
    @Override
    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain fc) throws ServletException, IOException {
        ...
        if(username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            //가져온 토큰이 유효한지와 토큰에 포함된 자용자의 이름이 userDetails의 사용자 이름과 동일한지 검사
            if(jwtTokenProvider.validateToken(jwt, userDetails.getUsername())){
                ...
                //생성된 인증객체를 SecurityContextHolder 의 현재 컨텍스트에 저장
